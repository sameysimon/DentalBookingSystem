    ...
    <div class="Content">
        ...
        <?php
            ...
            if (isset($_POST["appointment"]) == true) {
                ...
                $stmt = queryDatabase("SELECT startTime AS Time FROM Appointment WHERE appointmentID = '" . $_POST["appointment"] . "'");
                $result = $stmt->fetch();
                $time = $result["Time"];
                echo "$time";
                //Check if there's an appointment using this equipment at this time:
                $stmt = queryDatabase("SELECT appointmentID FROM Appointment WHERE equipmentID = '" . $_GET["equipmentID"] . "' AND (startTime <= '$time' AND endTime >= '$time')");
                echo "input3";
            ...
            if (isset($_POST["appointment"]) == false || $success == false) {
                ...
                $stmt = queryDatabase('SELECT Patient.name AS Name, Appointment.startTime AS Time, Appointment.appointmentID AS ID
                FROM Patient, Appointment
                WHERE (Appointment.status <> "Complete" OR Appointment.status <> "Canceled") AND Appointment.dentistID = "' . $_SESSION["username"] . '" AND Appointment.PatientID = Patient.PatientID');
